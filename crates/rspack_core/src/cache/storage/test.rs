...
pub struct FsStorage {
  location: PathBuf,
  inner: Mutex<HashMap<String, HashMap<Vec<u8>, Option<Vec<u8>>>>>,
}
...
impl Storage for FsStorage {
  ...
  fn set(&self, scope: &str, key: Vec<u8>, value: Vec<u8>) {
    ...
    let scope_map = inner.entry(String::from(scope)).or_default();
...
fn save(location: PathBuf, data: HashMap<String, HashMap<Vec<u8>, Option<Vec<u8>>>>) {
