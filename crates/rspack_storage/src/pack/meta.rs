...
impl ScopeMeta {
  ...
  pub fn write(&self, dir_path: &PathBuf) -> Result<()> {
    let file_path = dir_path.join("cache_meta");
    if file_path.exists() {
  ...
  pub fn read(dir_path: &PathBuf) -> Result<Self> {
    let meta_file_path = dir_path.join("cache_meta");
